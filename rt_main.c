...
extern void sound_callback2(void *buf, unsigned int reqn);
...
#if USE_LIBTIMIDITY
    pspAudioInit();
    pspAudioSetChannelCallback(1, (void *)&sound_callback2, NULL);
#endif
...
void starup_selector() {
	...
	SceCtrlData cpad;
	while(select!=1) {
	pspDebugScreenSetBackColor(0x00550000);
        pspDebugScreenClear();
	printF("\n\n\n\t\t\t RISE OF THE TRIAD version 1.0\n");
	printF("\t\t\t ==============================\n\n\n\n");
	pspDebugScreenSetBackColor(0x00005500);
	printF("\n\t\tSELECT FROM FOLLOWING STARTUP OPTIONS:\t\t\t\t\n");
...
return;
	...
	while (1) {
			...
			if (option_switch==0)
                ...
                if ((cpad.Buttons & PSP_CTRL_RIGHT) || (cpad.Lx <= 0x30)){
			if (option_switch==0) {
				if(cpu_speed<2)
					cpu_speed++;
			}else{
			...
			break;
                ...
                if ((cpad.Buttons & PSP_CTRL_LEFT) || (cpad.Lx >= 0xD0)){
			if (option_switch==0) {
				if(cpu_speed>0)
					cpu_speed--;
			}else{
