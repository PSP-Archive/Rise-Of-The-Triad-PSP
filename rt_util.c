...
/*
 *
 * Set the SDL_Color map for the screen (with brightness)
 *
 */

void SetSDLColors(void)
{
#if 0
   SDL_Color cmap[256];
   int i;
   ...
   for (i = 0; i < 256; i++)
   {
           cmap[i].r = svcmap[i].r; // + (Uint8)((255 - (Uint32)svcmap[i].r) * 100 / brightness);
           cmap[i].g = svcmap[i].g; // + (Uint8)((255 - (Uint32)svcmap[i].g) * 100 / brightness);
           cmap[i].b = svcmap[i].b; // + (Uint8)((255 - (Uint32)svcmap[i].b) * 100 / brightness);
   }

   SDL_SetColors (SDL_GetVideoSurface(), cmap, 0, 256);
#else
   SDL_SetColors (SDL_GetVideoSurface(), svcmap, 0, 256);
#endif
}
...
void GetaPalette (byte *palette)
{
	...
	for (i = 0; i < 256; i++, palette+=3) {
		palette[0] = svcmap[i].r;
		palette[1] = svcmap[i].g;
		palette[2] = svcmap[i].b;
	}
#endif
}
...
void SetaPalette (byte *pal)
{
   ...
   for (i = 0; i < 256; i++)
   {
       svcmap[i].r = pal[i*3+0];
       svcmap[i].g = pal[i*3+1];
       svcmap[i].b = pal[i*3+2];
   }
...
void VL_FillPalette (int red, int green, int blue)
{
   ...
   for (i = 0; i < 256; i++)
   {
           svcmap[i].r = (Uint8)(red << 2);
           svcmap[i].g = (Uint8)(green << 2);
           svcmap[i].b = (Uint8)(blue << 2);
   }
...
void VL_SetColor  (int color, int red, int green, int blue)
{
   ...
   OUTP (PEL_DATA,blue);
#else
   svcmap[color].r = (Uint8)(red << 2);
   svcmap[color].g = (Uint8)(green << 2);
   svcmap[color].b = (Uint8)(blue << 2);
...
void VL_GetColor  (int color, int *red, int *green, int *blue)
{
   ...
   *blue  = inp (PEL_DATA);
#else
   *red   = svcmap[color].r >> 2;
   *green = svcmap[color].g >> 2;
   *blue  = svcmap[color].b >> 2;
   //STUB_FUNCTION;
#endif
}
...
void VL_SetPalette (byte *palette)
{
   ...
   for (i = 0; i < 256; i++)
   {
	   svcmap[i].r = gammatable[(gammaindex<<6)+(*palette++)] << 2;
	   svcmap[i].g = gammatable[(gammaindex<<6)+(*palette++)] << 2;
	   svcmap[i].b = gammatable[(gammaindex<<6)+(*palette++)] << 2;
   }
...
void VL_GetPalette (byte *palette)
{
   ...
   for (i = 0; i < 768; i++)
	...
	for (i = 0; i < 256; i++, palette+=3) {
		palette[0] = svcmap[i].r >> 2;
		palette[1] = svcmap[i].g >> 2;
		palette[2] = svcmap[i].b >> 2;
	}
#endif
}
