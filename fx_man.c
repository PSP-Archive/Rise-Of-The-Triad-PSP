...
void MUSIC_SetVolume(int volume)
{
     if(midisong!=NULL)
        {
#if USE_LIBTIMIDITY
                mid_song_set_volume(midisong, (int)(( (float)volume/255.0)*100.0));
#else
...
//  sint8 buffer[4096];
  ...
  options.buffer_size = 4096 / (bits * channels / 8);
  midisong = mid_song_load (stream, &options);
  mid_istream_close (stream);
  songlength=mid_song_get_total_time(midisong);
   ...
   currentSongPosition=0;
    ...
    //music_musicchunk = Mix_LoadMUS(".rott/tmpsong.mod");
    if (music_musicchunk == NULL) {
        return MUSIC_Error;
    }
    printf("Mix_LoadMUS returned no error\n");
    Mix_PlayMusic(music_musicchunk, (loopflag == MUSIC_PlayOnce) ? 0 : -1);
#endif
...
#if USE_LIBTIMIDITY
void sound_callback2(void *buf, unsigned int _reqn)
{
        memset(buf, 0, _reqn*4);
        if(song_is_playing)
        {
                mid_song_read_wave (midisong, buf, _reqn*4);

                //loop it
                currentSongPosition=mid_song_get_time(midisong);
